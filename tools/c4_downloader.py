...
def main():
    ...
    pool.map(
        ...
        [local_path for _, local_paths in urls.items() for local_path in local_paths]
    )
    for local_path in [local_path for _, local_paths in tqdm(c4_urls.items()) for local_path in local_paths]:
